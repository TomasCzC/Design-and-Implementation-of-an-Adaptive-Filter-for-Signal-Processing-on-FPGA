...
from PyQt5.QtWidgets import QMainWindow, QWidget, QLabel, QComboBox, QDoubleSpinBox, QSpinBox, QCheckBox, QPushButton, QGridLayout, QHBoxLayout, QVBoxLayout, QTableWidget, QTableWidgetItem, QFileDialog, QMessageBox
from PyQt5.QtCore import Qt
from .canvases import MplCanvas, FftCanvas
...
try:
    ...
    try:
        ...
        PARAMS = cfg2.PARAMS
        LIMITS = cfg2.LIMITS
        PRESETS = cfg2.PRESETS
    except Exception:
        PARAMS = globals().get("PARAMS", {})
        LIMITS = globals().get("LIMITS", {})
        PRESETS = globals().get("PRESETS", {})

class MainWin(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Adaptive Filters (padasip) – GUI (safe mode)")
        ...
        self.spin_nt = QSpinBox(); self.spin_nt.setRange(1, 512); self.spin_nt.setValue(32)
        grid.addWidget(self.spin_nt, r, 1)

        r += 1
        grid.addWidget(QLabel("fs [Hz]"), r, 0)
        self.spin_fs = QDoubleSpinBox(); self.spin_fs.setRange(100.0, 192000.0); self.spin_fs.setValue(2000.0)
        grid.addWidget(self.spin_fs, r, 1)

        r += 1
        grid.addWidget(QLabel("f0 [Hz]"), r, 0)
        self.spin_f0 = QDoubleSpinBox(); self.spin_f0.setRange(0.1, 5000.0); self.spin_f0.setValue(100.0)
        grid.addWidget(self.spin_f0, r, 1)

        r += 1
        grid.addWidget(QLabel("T [s]"), r, 0)
        self.spin_T = QDoubleSpinBox(); self.spin_T.setRange(0.05, 20.0); self.spin_T.setValue(0.8)
        grid.addWidget(self.spin_T, r, 1)

        r += 1
        grid.addWidget(QLabel("Noise mean"), r, 0)
        self.spin_mean = QDoubleSpinBox(); self.spin_mean.setRange(-2.0, 2.0); self.spin_mean.setValue(0.0); self.spin_mean.setSingleStep(0.01)
        grid.addWidget(self.spin_mean, r, 1)

        r += 1
        grid.addWidget(QLabel("Noise std"), r, 0)
        self.spin_std = QDoubleSpinBox(); self.spin_std.setRange(0.0, 2.0); self.spin_std.setValue(0.1); self.spin_std.setSingleStep(0.01)
        ...
        r += 1
        self.btn_run = QPushButton("Run"); grid.addWidget(self.btn_run, r, 0)
        self.btn_save = QPushButton("Save Figures…"); grid.addWidget(self.btn_save, r, 1)
        ...
        self.tbl.setHorizontalHeaderLabels(["MSE_end", "EMSE", "J_min", "Misadj", "SNR_in [dB]", "SNR_out [dB]", "\u0394SNR [dB]", "N90%"])
    ...
    def _refresh_main_presets(self):
        name = self.cmb_alg.currentText() if isinstance(self.cmb_alg, QComboBox) else "LMS"
        self.cmb_preset_main.clear()
        self.cmb_preset_main.addItems(list(PRESETS.get(name, {"Default": {}}).keys()))
    ...
    def apply_preset_main(self):
        ...
        for k, v in preset.items():
            if k not in PARAMS[alg]:
                continue
            lo, hi = LIMITS[alg][k]
            PARAMS[alg][k] = int(round(max(min(v, hi), lo))) if k == "order" else float(max(min(v, hi), lo))
        self.run_once()
    ...
    def run_once(self):
        ...
        t, s, x = make_signals(fs=fs, f0=f0, T=T, noise_mean=mean, noise_std=std, anc=anc, seed=seed)

        L = len(t)
        if nt > L:
            QMessageBox.warning(self, "Parameter error", f"Taps nt ({nt}) larger than signal length ({L}). Reducing nt.")
            nt = L
        ...
        try:
            ...
            QMessageBox.warning(self, "Filter error", f"{alg} failed during run:\n{ex}\n\nParams: {params}")
            return
        ...
        if is_diverged(y, e):
            QMessageBox.warning(self, "Divergence detected", "Filter outputs are non-finite or exceed safety limits.\nReduce mu or change parameters.")
            return

        print(f"[DEBUG] max|y|={np.max(np.abs(y)):.6e}, max|e|={np.max(np.abs(e)):.6e}")
        ...
        try:
            ...
            self.update_table(m)
            self._last_state = dict(t=t, s=s, x=x, y=y, e=e, nt=nt, fs=fs, alg=alg, anc=anc, w=w, params=params)
        except Exception as ex:
            QMessageBox.warning(self, "Plot error", f"Plotting failed: {ex}")
    ...
    def redraw_main_plots(self, t, s, x, y, e, nt, title, anc):
        ...
        c.ax1.set_title(f"{title} – Inputs"); c.ax1.grid(True); c.ax1.legend()
        ...
        c.ax2.set_title("Output vs reference"); c.ax2.grid(True); c.ax2.legend()

        c.ax3.plot(t[nt - 1:], e, label="e")
        c.ax3.set_title("Error"); c.ax3.grid(True); c.ax3.legend()

        mse_db = safe_log10_of_square(e)
        mse_db = moving_avg(mse_db, max(1, int(0.05 * len(mse_db))))
        c.ax4.plot(t[nt - 1:], mse_db, label="MSE (dB)")
        c.ax4.set_title("MSE (dB)"); c.ax4.grid(True); c.ax4.legend()
    ...
    def redraw_fft(self, t, s, x, y, nt, fs, title, anc):
        ...
        f1, Xin = fft_mag(xin, fs) if xin.size else (np.array([]), np.array([]))
        f2, Y = fft_mag(y, fs) if len(y) else (np.array([]), np.array([]))
        f3, S = fft_mag(sref, fs) if sref.size else (np.array([]), np.array([]))
    ...
    def save_figures(self):
        if not hasattr(self, "_last_state") or self._last_state is None:
            QMessageBox.information(self, "No data", "No figures to save. Run simulation first.")
            return
        fn, _ = QFileDialog.getSaveFileName(self, "Save base name", "adaptive_output", "PNG (*.png)")
        if not fn:
            return
