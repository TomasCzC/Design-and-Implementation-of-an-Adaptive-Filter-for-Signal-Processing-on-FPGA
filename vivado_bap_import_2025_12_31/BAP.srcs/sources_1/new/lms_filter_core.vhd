-- ============================================================================
-- Entity: lms_filter_core.vhd
-- LMS adaptive filter, synthesizable Q16.16 implementation
-- ============================================================================
...
entity lms_filter_core is
    ...
    port (
        ...
        w_out_addr   : out integer range 0 to NUM_TAPS-1;
        w_out_data   : out fix_point;
...
architecture rtl of lms_filter_core is
    ...
    begin
        if reset_n = '0' then
            ...
            if valid_in = '1' and update_active = '0' then
                ...
                if freeze = '0' then
                    mu_e := safe_multiply(mu, e_reg);
                    weight_delta := safe_multiply(mu_e, x_delay(tap_cnt));
                    weights(tap_cnt) <= sat_add(weights(tap_cnt), weight_delta);
                end if;

                w_out_addr <= tap_cnt;
                w_out_data <= weights(tap_cnt);
